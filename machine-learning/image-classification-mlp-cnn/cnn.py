...
from torch.nn import Linear, Conv2d, BatchNorm2d, Dropout, ReLU, LogSoftmax, Sigmoid, Tanh, Module, CrossEntropyLoss, MSELoss, Softmax, functional
...
class Network(Module):
    def __init__(self):
        ...
        self.conv1BatchNormalization = BatchNorm2d(6)
        self.convActivation1 = convLayerActivation()

        self.conv2 = Conv2d(in_channels=6, out_channels=12, kernel_size=5)
        self.conv2BatchNormalization = BatchNorm2d(12)
        self.convActivation2 = convLayerActivation()

        self.dropout1 = Dropout(0.75)

        self.hidden1 = Linear(12*4*4, 128)
        self.activation1 = hiddenLayerActivation()

        self.hidden2 = Linear(128, 64)
        self.activation2 = hiddenLayerActivation()

        self.outout = Linear(64, 10)
        self.outputActivation = outputLayerActivation()


    def forward(self, X):
        X = self.conv1(X)
        X = self.conv1BatchNormalization(X)
        X = self.convActivation1(X)
        X = functional.max_pool2d(X, kernel_size=2, stride=2)

        X = self.conv2(X)
        X = self.conv2BatchNormalization(X)
        X = self.convActivation2(X)
        X = functional.max_pool2d(X, kernel_size=2, stride=2)
