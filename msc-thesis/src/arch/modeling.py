...
class ARCHEncoder(nn.Module):
    def __init__(self, config):
        ...
        print("ARCH encoder with %d l_layers, %d x_layers, and %d r_layers." %
        ...
        self.x_layers = nn.ModuleList(
            [ARCHXLayer(config) for _ in range(self.num_x_layers)]
        )
...
class ARCHModel(BertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.embeddings = BertEmbeddings(config)
        self.encoder = ARCHEncoder(config)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        # extec edition to use multiGPU
        # dataType = next(self.parameters()).dtype
        dataType = torch.float32
        extended_attention_mask = extended_attention_mask.to(dtype=dataType) # fp16 compatibility
        # print(dataType)
...
class ARCHPretraining(BertPreTrainedModel):
    def __init__(self,
                 config,
                 task_mask_lm=True,
                 task_matched=True,
...
class ARCHFeatureExtraction(BertPreTrainedModel):
    """
