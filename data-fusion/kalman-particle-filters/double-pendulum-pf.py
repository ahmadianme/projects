...
particleCount = 1000
particles = np.tile(xHat_1, (particleCount, 1))

weights = np.zeros(particleCount)



for step in tqdm(range(stepCount)):
    for i in range(particleCount):
        particles[i] += stateTransition(particles[i], deltaT) + np.random.multivariate_normal(np.zeros(n), stdQ)

    z = measurement(groundTruthX)
    for i in range(particleCount):
        error = z - measurement(particles[i])
        weights[i] = np.exp(-(error @ RInverse @ error.T) / 2)

    weights /= np.sum(weights)

    particles = particles[np.random.choice(range(particleCount), size=particleCount, replace=True, p=weights)]

    xHatAll.append(np.mean(particles, axis=0))
