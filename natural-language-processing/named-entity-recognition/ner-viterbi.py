...
NLTKDataset = list(nltk.corpus.treebank.tagged_sents())

neChunks = [nltk.ne_chunk(sentence) for sentence in NLTKDataset]

dataIOBTaggedTmp = [tree2conlltags(neChunk) for neChunk in neChunks]

NLTKDataset = []

for sentence in dataIOBTaggedTmp:
    sentenceTagged = []
    for word in sentence:
        sentenceTagged.append((word[0], word[2]))

    NLTKDataset.append(sentenceTagged)
...
def Viterbi(words, trainItems = trainTaggedWords):
    ...
    with tqdm(total=len(words)) as pbar:
        for key, word in enumerate(words):
            ...
            for tag in tagList:
                if key == 0:
                    transitionProbability = tagsDataFrame.loc['O', tag]
                else:
