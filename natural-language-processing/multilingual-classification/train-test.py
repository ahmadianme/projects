...
if language == 'fa':
    trainDF['sentence'] = trainDF['targets']
    validationDF['sentence'] = validationDF['targets']
    testDF['sentence'] = testDF['targets']
elif language == 'en':
    trainDF['sentence'] = trainDF['source']
    validationDF['sentence'] = validationDF['source']
    testDF['sentence'] = testDF['source']
else:
    trainDF['sentence'] = trainDF['source'] + '[SEP]' + trainDF['targets']
    validationDF['sentence'] = validationDF['source'] + '[SEP]' + validationDF['targets']
    testDF['sentence'] = testDF['source'] + '[SEP]' + testDF['targets']
...
for epoch in tqdm(range(1, epochs+1)):
    ...
    if epoch % 5 == 0:
        torch.save(model.state_dict(), f'q2/finetuned_'+ modelSaveName +'_epoch_{epoch}.model')
    ...
    if validationLoss < bestValLoss:
        torch.save(model.state_dict(), f'q2/finetuned_'+ modelSaveName +'_BEST.model')
...
model.load_state_dict(torch.load('q2/finetuned_'+ modelSaveName +'_BEST.model', map_location=torch.device('cpu')))
