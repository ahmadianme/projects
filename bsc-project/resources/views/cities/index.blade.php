...
@section('content')
	<div class="uk-grid" style="margin-bottom: 25px;">
		<div class="uk-width-1-2">                    
			<a href="{{ URL::to('cities/new') }}" class="md-btn md-btn-primary md-btn-wave-light waves-effect waves-button waves-light" href="form.html">جدید</a>
		</div>
			...
			<table class="uk-table uk-table-nowrap table_check">
				<thead>
					<tr>
						...
						<th class="uk-width-1-10 uk-text-center">کشور</th>
				...
				<tbody>
					@foreach ($records as $record)
						<tr>
							<td>{{ $record->name }}</td>
							<td class="uk-text-center">{{ $record->country->name }}</td>
							<td class="uk-text-center">{{ isset($record->user) ? $record->user->name . ' ' . $record->user->lname : '-' }}</td>
							<td class="uk-text-center">
								<button onclick="window.location='{{ URL::to('cities/edit') }}/{{ $record->id }}';" class="md-btn md-btn-flat md-btn-wave waves-effect waves-button" style="padding: 0; min-width: 40px;"><i class="md-icon material-icons">&#xE254;</i></button>
								<form action="{{ URL::to('cities') }}/{{ $record->id }}" method="POST" id="deleteForm{{ $record->id }}" style="display: inline;">
						            {{ csrf_field() }}
						            {{ method_field('DELETE') }}
